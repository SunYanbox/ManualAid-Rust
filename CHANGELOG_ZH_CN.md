...
- `@` 路径补全缓存在超过 30 秒（`complete/paths.rs` 中的 `REFRESH_INTERVAL`）或因某个引用已不成立而被置为过期时重建：`filter` 先调用 `refresh_if_stale`，当 `last_scan` 早于该间隔或已被清空时用已保存的根路径重新扫描，同时刷新可见遍历与浏览过的目录缓存，使 loop 运行期间新建、删除或移动的文件出现在后续建议中；`run_path_actions` 返回 `PathRoundOutcome`，上报是否有 `@token` 未能解析，loop 据此调用新增的 `invalidate`，使这条过期条目在下一次 `@` 查询即被丢弃，而不必等到间隔结束；从未扫描过的实例（根为空）与仍在间隔内的缓存不做处理，刷新保持惰性——重建发生在之后的那次查询而非上报当下，也不引入后台任务或每次按键重扫
...
- 提示词中渲染的 JSON 工具调用模板（`JsonCodeblockParser::tool_call_template`）现在是合法 JSONC：此前参数循环把每行 `"name": value` 直接拼接，导致 `read` 这类多参数工具的示例缺少逗号；现在各参数以逗号分隔（逗号置于值之后、`// optional` 注释之前，末位参数不加逗号以避免尾逗号），并把末尾的 `Note:` 行改为 `// Note:` 注释，去掉注释后即为严格 JSON
...
- 修复补全建议面板的显示错乱：`render` 原先按候选条数计算面板高度，而单行可能因标签或描述过长折到第二行，导致实际绘制行数多于记账行数、`MoveUp` 少移、光标落在建议区内并在下次渲染覆盖残影。现按终端宽度把每一行压到一行内（CJK 按两列计宽，新增 `render_suggestion_row`/`truncate_to_width`/`char_width`），并新增 `PanelLayout` 承载终端宽度限制；面板高度保持原有的 `MAX_SUGGESTIONS` 上限，不随光标位置变化，因为终端滚动会把已绘制行与光标一起上移，回到输入行的相对移动依然正确

## [0.13.0] - 2026-09-10

### 新增

- 内联命令新增 `/compress`（复制压缩会话提示词）、`/plan` 与别名 `/p`（复制计划模式提示词）、`/build` 与别名 `/b`（复制切换执行模式提示词）、`/skills`（复制已启用 SKILL 列表）；`/tools` 由分页打印改为复制已启用工具的完整列表模板；`/help` 与 Tab 补全同步列出新命令
- 复制提示词二级菜单新增“复制已启用 SKILL 列表”项，并把“复制当前已启用工具列表”改为复制完整工具列表模板（含描述、参数与调用模板）；工具与 SKILL 列表的复制内容统一以 `<system-reminder>` 块包裹，`manualaid-ws` 新增 `render_tools_list_reminder` 与 `render_skills_list` 两个公开渲染函数，无已启用 SKILL 时以本地化占位文案填充
...
- 新增 `loop_cli/progress.rs`：工具执行阶段的单行进度显示 `(Ns) TOOL1, TOOL2, ...`，按状态着色（未执行=灰、执行中=黄、成功=绿、失败=红），以 `tokio::select!` 让执行 future 与 200ms ticker 竞争以持续刷新秒数；`ProgressLine` 在测试构建、console 捕获或非终端 stdout 下整体禁用，`suspend`/`finish` 支持中途分页 diff 时擦除重绘并在执行结束时定格换行；渲染抽为纯函数便于单测
