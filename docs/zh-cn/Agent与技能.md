...
- Skill 工具包含必填参数 `skill` 和可选参数 `args`（传给技能主机的字符串，当前随 `invoke_skill` JSON 头部返回）。`skill` 取值应来自 `<available_skills>` 列表所示的名称——该列表展示最短唯一形式（无冲突时仅技能名，冲突时带 `<agent_dir>-` 或 `<scope>-<agent_dir>-` 前缀）；传入完整稳定唯一名称亦可解析。
- 支持热加载，不支持版本管理。
- 技能与 Agent 无强制绑定关系。
- 技能默认启用/禁用规则：
