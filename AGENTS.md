...
两者都是提交消息的下游，各自面向不同读者，**禁止把提交正文原样搬运**。信息分工：动机归提交消息，行为变化归 CHANGELOG，聚合视图与验证记录归 PR 描述。

**CHANGELOG 条目**：
- 一条只写一个可独立感知的行为变化，超过一个即拆分；中文单条超过约 150 字即为拆分信号。
- 用户版只写行为结果；开发者版可另加模块路径、类型名、配置项名等定位信息。
- 不写控制流走读、函数签名、内部数据结构，也不写动机叙述。
- 破坏性变更、迁移要求与新增配置开关必须写明。

**PR 描述**：
- 必含概述（引用关联 issue）、动机、影响边界（哪些行为改变、哪些不变）与验证记录（跑过的检查）。
- 破坏性变更与迁移说明单独标出。
- 不写逐文件代码走读、提交消息拼盘、CHANGELOG 全文粘贴、敏感信息与过程流水账。
