...
- **原子提交**：一个提交只做一件事；无关改动拆分为多个提交。
- **消息承载变更类型与变更内容**：主题行按约定式提交写明 type 与 scope，正文说明改动内容及其影响与风险。
- **消息承载动机与背景**：说明为什么要改，即当前行为的问题或诉求；这部分无法从 diff 读出，必须写进消息。
- **How 保留在 diff 中**：不复述实现细节（逐文件的代码改动、函数内部逻辑、参数增删的逐条说明）；这些由 diff 自身承载，在消息里重复只会随代码演进而失准。
