...
async fn enabled_project_skill_returns_body() {
    ...
    // JSON serialises each `\` as `\\`, so match on the path's file name
    // component which is unambiguous across platforms.
    assert!(
        result.output.contains("demo-skill"),
        "output should contain skill path (got `{}`)",
        result.output
    );
    reset_skills();
