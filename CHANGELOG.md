...
- The round-level truncation warning no longer asks the agent to adjust its tool call parameters (`i18n/locales/common.{en,zh-CN}.toml`)
...
- The system prompt is audited and repaired for ambiguity in both languages: internal contradictions are removed (the `# ToolCall` block now names the `func_calls` fence instead of `tool_calls`, matching every other reference; `plan-mode-rule` no longer both requires and forbids pasting the plan), undefined and dangling references are dropped (`<command-name>`, the `/SKILL` spelling, the `PowerShell` alias table in an environment whose shell is detected at runtime), the file-handling rules are tightened so a terminal route never substitutes for read/write/edit, and the escaping requirement moves to `cli.prompt.func_calls_notes`, and `platform-notes` / `coreutils-notes` are rewritten to describe what CoreUtils actually provides instead of restating generic platform advice
- The `# ToolCall` block now accepts a JSON array of call objects inside a single `func_calls` code block, and the prompt asks the model to prefer one array when a reply issues many calls; the parser already handled this shape, so only the prompt text changed
- `tool_calling_format_description` and `copy_tool_format_with_provider` no longer take a `Config`: the `%{format}` placeholder in the `cli.prompt.format_desc` text used to print the current tool-call format's short name, a name this round judged unnecessary, so the placeholder is removed and the text now points at the example below; the parameter that fed it is gone from both signatures
...
- The copy-prompt submenu adds a "copy enabled skills list" entry and upgrades "copy currently enabled tools list" to copying the full tools list template (descriptions, parameters and call templates); copied tools and skills lists are uniformly wrapped in a `<system-reminder>` block, with two new public renderers `render_tools_list_reminder` and `render_skills_list` in `manualaid-ws`, the latter filling a localized placeholder when no skill is enabled.
